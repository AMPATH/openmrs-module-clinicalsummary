 ...
 * 
 */
public class DNAPCRRule implements Rule {
	
	private static final Log log = LogFactory.getLog(DNAPCRRule.class);
	...
	public Result eval(LogicContext context, Patient patient, Map<String, Object> parameters) throws LogicException {
		...
		LogicCriteria obsCriteria = service.parseToken(SummaryDataSource.CONCEPT).equalTo(StandardConceptConstants.DNA_PCR_NAME);
		
		Result obsResults = context.read(patient, service.getLogicDataSource("summary"), obsCriteria.and(encounterCriteria));
		
		if (log.isDebugEnabled())
			log.debug("dna pcr observations for patient: " + patient.getPatientId() + " is: " + obsResults);
