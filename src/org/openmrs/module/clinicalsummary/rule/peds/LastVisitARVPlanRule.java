 ...
 * 
 */
public class LastVisitARVPlanRule implements Rule {
	
	private static final Log log = LogFactory.getLog(LastVisitARVPlanRule.class);
	...
	public Result eval(LogicContext context, Patient patient, Map<String, Object> parameters) throws LogicException {
		...
		LogicCriteria obsCriteria = service.parseToken(SummaryDataSource.CONCEPT).equalTo(StandardConceptConstants.ARV_PLAN_LAST_VISIT_NAME);
		
		Result obsResults = context.read(patient, service.getLogicDataSource("summary"), obsCriteria.and(encounterCriteria));
		
		if (log.isDebugEnabled())
			log.debug("Patient: " + patient.getPatientId() + ", last arv plan: " + obsResults);
