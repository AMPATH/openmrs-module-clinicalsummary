 ...
 * 
 */
public class ARVSideEffectRule implements Rule {
	
	private static final Log log = LogFactory.getLog(ARVSideEffectRule.class);
	...
	public Result eval(LogicContext context, Patient patient, Map<String, Object> parameters) throws LogicException {
		...
		LogicCriteria obsCriteria = service.parseToken(SummaryDataSource.CONCEPT).equalTo(StandardConceptConstants.ARV_SIDE_EFFECT_NAME);
		
		Result obsResults = context.read(patient, service.getLogicDataSource("summary"), obsCriteria.and(encounterCriteria));
		
		if (log.isDebugEnabled())
			log.debug("arv side effect observations for patient: " + patient.getPatientId() + " is: " + obsResults);
		
		for (Result obsResult : obsResults)
	        if (!result.contains(obsResult.toConcept()))
