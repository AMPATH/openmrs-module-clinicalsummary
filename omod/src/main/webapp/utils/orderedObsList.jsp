...
<script>
	...
	$j(function() {
		...
		$j("#result").click(function(event) {
			$j(datatable.fnSettings().aoData).each(function () {
				$j(this.nTr).removeClass('row_selected');
			});
			$j(event.target.parentNode).addClass('row_selected');
		});

		datatable = $j('#result').dataTable({
						...
						<li>
							...
							<select name="status" id="status">
								...
								<c:forEach var="status" items="${statuses}" varStatus="varStatus">
									<option value="${status.key.value}" desc="${status.value}">${status.value}</option>
								</c:forEach>
							</select>
						</li>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="display" id="result">
				<thead>
					<tr>
						...
						<th class="header"><spring:message code="clinicalsummary.ordered.report.id"/></th>
