...
public class HighRiskExpressCareReminderRule extends EvaluableRule {

	private static final Log log = LogFactory.getLog(HighRiskExpressCareReminderRule.class);

	public static final String TOKEN = "Adult High Risk Express Care Referral Reminder";
	...
	protected Result evaluate(final LogicContext context, final Integer patientId, final Map<String, Object> parameters) throws LogicException {
		...
		if (CollectionUtils.isNotEmpty(obsClusterResults)) {
			...
			if (latestClusterResult.toNumber() < 100) {
				// prepare the referrals parameters
				...
				if (CollectionUtils.isEmpty(referralResults)
						|| referralResults.getResultDate().before(latestClusterResult.getResultDate())) {
					parameters.put(EvaluableConstants.ENCOUNTER_TYPE, Arrays.asList(EvaluableNameConstants.ECHIGHRISK));
					EncounterWithRestrictionRule encounterWithRestrictionRule = new EncounterWithStringRestrictionRule();
					Result highRiskEncounterResults = encounterWithRestrictionRule.eval(context, patientId, parameters);
					...
					if (CollectionUtils.isEmpty(arvResults) && CollectionUtils.isEmpty(highRiskEncounterResults))
