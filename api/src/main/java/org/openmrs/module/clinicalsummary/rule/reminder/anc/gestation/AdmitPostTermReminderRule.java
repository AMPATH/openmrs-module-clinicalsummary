...
public class AdmitPostTermReminderRule extends EvaluableRule {

	private static final Log log = LogFactory.getLog(AdmitPostTermReminderRule.class);

	public static final String TOKEN = "Admit Post Term Reminder";
	...
	protected Result evaluate(final LogicContext context, final Integer patientId, final Map<String, Object> parameters) throws LogicException {
		...
		if (CollectionUtils.isNotEmpty(pregnancyDateResults)) {
			...
			// if the gestational is older than 41 weeks
			if (gestationalAge >= 41) {
				...
				// display the reminder when no hospital referrals are ordered
				if (CollectionUtils.isNotEmpty(referralResults)) {
					Result referralResult = referralResults.latest();
					Concept obstetricConcept = CacheUtils.getConcept(EvaluableNameConstants.ADMITTED_TO_HOSPITAL);
