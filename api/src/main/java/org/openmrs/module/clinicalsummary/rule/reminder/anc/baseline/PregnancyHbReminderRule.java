...
public class PregnancyHbReminderRule extends EvaluableRule {

	private static final Log log = LogFactory.getLog(PregnancyHbReminderRule.class);

	public static final String TOKEN = "Pregnancy Hb Reminder";
	...
	protected Result evaluate(final LogicContext context, final Integer patientId, final Map<String, Object> parameters) throws LogicException {
		...
		if (CollectionUtils.isNotEmpty(pregnancyDateResults)) {
			...
			// fetch the latest hemoglobin results
			parameters.put(EvaluableConstants.OBS_FETCH_SIZE, 1);
			parameters.put(EvaluableConstants.OBS_CONCEPT, Arrays.asList(EvaluableNameConstants.HEMOGLOBIN));
			Result hemoglobinResults = obsWithRestrictionRule.eval(context, patientId, parameters);
			if (CollectionUtils.isEmpty(hemoglobinResults) || hemoglobinResults.getResultDate().before(pregnancyDate)) {
