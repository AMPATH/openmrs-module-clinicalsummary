...
public class AntenatalReminderStudyRule extends AbstractPostProcessorRule {

	private static final Log log = LogFactory.getLog(AntenatalReminderStudyRule.class);

	public static final String TOKEN = "Antenatal Reminder Study";

	private static final String RANDOM_NUMBER_PROPERTY = "clinicalsummary.antenatal.study.randomizer";
	...
	protected Boolean applicable(final Map<String, Object> parameters) {
		...
		for (Mapping mapping : mappings) {
			...
			if (StringUtils.equalsIgnoreCase(encounterType.getName(), EvaluableNameConstants.ENCOUNTER_TYPE_ANCINITIAL)
					|| StringUtils.equalsIgnoreCase(encounterType.getName(), EvaluableNameConstants.ENCOUNTER_TYPE_ANCRETURN))
				return Boolean.TRUE;
		}
