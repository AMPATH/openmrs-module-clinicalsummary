...
//        }
        ...
        Result element6AResult = element6ARule.eval(context, patientId, parameters);
        if (element6AResult.toBoolean()) {
            result.add(new Result("Excluded by E6A"));
            return result;
        }

        Element6BRule element6BRule = new Element6BRule();
        Result element6BResult = element6BRule.eval(context, patientId, parameters);
        if (element6BResult.toBoolean()) {
            result.add(new Result("Excluded by E6B"));
            return result;
        }
