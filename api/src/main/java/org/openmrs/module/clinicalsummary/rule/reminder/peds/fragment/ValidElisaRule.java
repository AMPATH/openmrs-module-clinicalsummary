...
/**
 * Valid elisa results are results that comes after the patient is 18 months old.
 * <p/>
 * Parameters: <ul> <li>[Optional] valueCoded: the possible answers for the Elisa</li> <li>[Optional] size: the size that should
 * be returned by the rule</li> <li>[Optional] order: the ordering of the list that should be returned by the rule</li> </ul>
 */
public class ValidElisaRule extends EvaluableRule {

	private static final Log log = LogFactory.getLog(ValidElisaRule.class);

	public static final String TOKEN = "Valid Elisa";
	...
	protected Result evaluate(final LogicContext context, final Integer patientId, final Map<String, Object> parameters) throws LogicException {
		...
		if (patient.getBirthdate() != null) {
			...
			Result validElisaResults = obsWithRestrictionRule.eval(context, patientId, parameters);

			for (Result validElisaResult : validElisaResults) {
				if (validElisaResult.getResultDate().after(validReferenceDate))
					result.add(validElisaResult);
			}
		}
